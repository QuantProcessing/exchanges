...
- [ ] SDK naming aligns with repository conventions. Confirm request helpers use repository-standard verbs, treat Backpack's `GetOrderBook` and `PlaceOrder` as the current baseline for shared SDK query/order naming, and treat generic `WsClient` packages as converged on `WSClient` while `WebsocketClient`, `BaseWsClient`, and `WsApiClient` remain explicitly deferred unless a documented protocol constraint requires a local deviation.
