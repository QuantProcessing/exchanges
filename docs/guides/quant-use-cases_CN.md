...
当前 funding snapshot providers：Binance Perp、Aster Perp、OKX Swap、Bybit
Linear、Bitget Perp、Hyperliquid Perp、Lighter、EdgeX、GRVT、StandX、Backpack、Nado。
策略在把 funding snapshot 用于 sizing/execution 前，应校验所需 funding 字段。mark/index prices
是单独的 market-data 输入，不嵌入 `model.FundingRate`。
