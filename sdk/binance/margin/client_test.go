...
func requireBinanceMarginLiveWrite(t *testing.T, vars ...string) *Client {
	t.Helper()
	required := append([]string{"BINANCE_API_KEY", "BINANCE_SECRET_KEY"}, vars...)
	testenv.RequireLiveWrite(t, binanceMarginLiveWriteFlag, required...)
	return newLivePrivateClient(t)
}

func marginEnvOrDefault(key, fallback string) string {
