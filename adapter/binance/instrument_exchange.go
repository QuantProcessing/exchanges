...
func (a *Adapter) marketSymbol(market exchanges.MarketRef) string {
	return normalizeInstrumentMarket(market, exchanges.QuoteCurrency(a.quoteCurrency), exchanges.MarketTypePerp).Symbol()
}

func (a *Adapter) paramsForMarket(market exchanges.MarketRef, params *exchanges.OrderParams) *exchanges.OrderParams {
	return paramsForMarket(market, exchanges.QuoteCurrency(a.quoteCurrency), exchanges.MarketTypePerp, params)
}
...
func (a *SpotAdapter) marketSymbol(market exchanges.MarketRef) string {
	return normalizeInstrumentMarket(market, exchanges.QuoteCurrency(a.quoteCurrency), exchanges.MarketTypeSpot).Symbol()
}

func (a *SpotAdapter) paramsForMarket(market exchanges.MarketRef, params *exchanges.OrderParams) *exchanges.OrderParams {
	return paramsForMarket(market, exchanges.QuoteCurrency(a.quoteCurrency), exchanges.MarketTypeSpot, params)
}
...
func normalizeInstrumentMarket(market exchanges.MarketRef, defaultQuote exchanges.QuoteCurrency, marketType exchanges.MarketType) exchanges.MarketRef {
	...
	if market.Base == "" {
		market = exchanges.ParseMarketRef(market.Symbol(), defaultQuote, marketType)
	}
	if market.Quote == "" {
