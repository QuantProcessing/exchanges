// Package bybit — OptionAdapter skeleton.
//
// This file declares the Bybit option adapter surface so consumers can
// compile against the OptionExchange interface even before the full
// implementation lands. Every method returns ErrNotSupported with a TODO.
//
// Reference implementation: binance/option_adapter.go.
// Bybit v5 uses category=option for option endpoints; instrument IDs look
// like "BTC-26DEC25-100000-C".
package bybit
...
// OptionAdapter is a not-yet-implemented Bybit options adapter.
...
func NewOptionAdapter(_ context.Context, opts Options) (*OptionAdapter, error) {
	...
	return &OptionAdapter{
		BaseAdapter: exchanges.NewBaseAdapter("BYBIT_OPTION", exchanges.MarketTypeOption, logger),
	}, nil
}
...
func notSupported(op string) error {
	return fmt.Errorf("bybit options %s: %w (TODO: implement against /v5/... category=option)", op, exchanges.ErrNotSupported)
}
