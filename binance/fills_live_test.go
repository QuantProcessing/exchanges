...
func TestPerpAdapter_WatchFillsLive(t *testing.T) {
	...
	buyFills := waitFills(t, fillCh, buyFilled.OrderID, buyFilled.ClientOrderID, exchanges.OrderSideBuy, buyFilled.FilledQuantity, 30*time.Second)
	require.NotEmpty(t, buyFills)
	for _, fill := range buyFills {
	...
	sellFills := waitFills(t, fillCh, sellFilled.OrderID, sellFilled.ClientOrderID, exchanges.OrderSideSell, sellFilled.FilledQuantity, 30*time.Second)
